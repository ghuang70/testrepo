...
options_list = [{'label': 'All Sites', 'value': 'ALL'}]+[{'label': site, 'value': site} for site in launchsite]
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        labels = ['0','1']
        values = [(filtered_df['class']==0).sum(), (filtered_df['class']==1).sum()]
        #filtered_df['flag'] = np.where(df['class'] > 0 , 'T','F')
